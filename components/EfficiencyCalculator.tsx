    ...
    <div className="py-32 bg-white relative overflow-hidden" id="roi">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        <div className="grid lg:grid-cols-2 gap-24 items-center">
          <div>
            <div className="inline-flex items-center gap-2 bg-white text-tzero-navy px-5 py-2 rounded-full text-[11px] font-black uppercase tracking-[0.25em] mb-8 border border-slate-200 shadow-sm">
              <Calculator size={14} className="text-tzero-blue" /> Simulador ZeroLoss
            </div>
            <h2 className="text-4xl lg:text-7xl font-black text-tzero-navy mb-8 tracking-tighter leading-[0.95]">Previsibilidade <br /> financeira <span className="text-tzero-blue">total.</span></h2>
            <p className="text-slate-500 text-lg mb-12 font-medium leading-relaxed max-w-lg">
              Pare de pagar por tentativas falhas. O modelo <span className="text-tzero-navy font-bold">ZeroLoss (CPC)</span> garante que seu orçamento se converta em contatos efetivos.
            </p>
          ...
          <div className="bg-white rounded-[3.5rem] p-12 lg:p-20 text-center shadow-[0_50px_100px_-20px_rgba(11,28,63,0.12)] relative border border-slate-100">
            <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-tzero-blue p-7 rounded-[2rem] shadow-2xl shadow-blue-500/30">
            ...
            <div className="text-6xl lg:text-8xl font-black text-tzero-navy mb-12 tracking-tighter">
            ...
            <button className="bg-tzero-blue text-white w-full py-8 rounded-[2rem] text-xl font-black uppercase tracking-[0.2em] shadow-2xl shadow-blue-500/20 hover:brightness-110 transition-all transform hover:-translate-y-1 flex items-center justify-center gap-4">
              Ativar ZeroLoss <ArrowRight size={24} />
            </button>
...
const Slider = ({ label, val, min, max, step, current, set }: any) => (
  <div className="space-y-6">
    <div className="flex justify-between font-black text-xs uppercase tracking-[0.25em]">
      ...
      <span className="text-tzero-navy font-bold">{val}</span>
    </div>
    <div className="relative group">
      <input 
        type="range" min={min} max={max} step={step} value={current} 
        onChange={(e) => set(Number(e.target.value))}
        className="w-full h-2 bg-slate-100 rounded-full appearance-none cursor-pointer outline-none transition-all accent-tzero-blue"
      />
    </div>
  </div>
);
